...
def main(unused_arg):
    ...
    if len(sys.argv) >= 2:
        ...
        x = tf.placeholder('float',[None,constants.IMG_SIZE,constants.IMG_SIZE,3])
        y = tf.placeholder('float',[None,constants.CNN_CLASSES])
        weights = {}
        biases = {}

        #first convolution layers
        weights['W_conv1'] = tf.Variable(tf.random_normal([3,3,3,constants.CNN_LAYER1]))
        ...
        magic_number = int(constants.IMG_SIZE * constants.IMG_SIZE * constants.CNN_LAYER1)
        ...
        if(sys.argv[1] == 'train'):
            ...
            with tf.Session() as sess:
                ...
                for epoch in range(constants.CNN_EPOCHS):
                    #get an image batch
                    batch_x,batch_y = featureReader.getSegmentBatch(constants.BATCH_SIZE)
                    eval_x,eval_y = featureReader.getSegmentBatch(constants.BATCH_SIZE)
        ...
        #testing method needs a saved check point directory (model)
            ...
            #read the image
            imgSegments = featureReader.cnn_readOneImg(sys.argv[2])
            ...
            with tf.Session() as sess:
                ...
                i = 1
                inputs = []
                for segment in imgSegments:
                    prediction = predict_op.eval({x:[segment]})
                    if(prediction == [0]):
                    ...
                    print('%i out of %i' % (i,len(imgSegments)))
                    i += 1
